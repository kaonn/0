...
\infern{
  ...
  V,H,R,F \; \vdash \irl{lam}(x : \tau.e) \Downarrow (V, x.e)^l,H' ,F'
}
...
\infern{
  ...
  V,H,R,F \; \vdash \pairexcst{x_1}{x_2} \Downarrow \pairexcst{v_1}{v_2}^l,H' ,F'
}

\infern{
  V(x) = \pairexcst{v_1}{v_2}^l\\
}{
  V,H,R,F \; \vdash \fstexcst{x} \Downarrow v_1,H ,F
}

\infern{
  V(x) = \pairexcst{v_1}{v_2}^l\\
}{
  V,H,R,F \; \vdash \sndexcst{x} \Downarrow v_2,H,F
}
...
\begin{mathpar}
\infer[\text{L:CONSTI}]{
  \Sigma; \emptyset \sststile{q}{q} n : int 
}{
  n \in \mathbb{Z}
}
