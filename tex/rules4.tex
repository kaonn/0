...
\begin{enumerate}
  \item $|F| - |F'| \le \Phi_{V,H}(\Gamma) +q - (\Phi_{H'}(v:B) + q')$
  \item $locs_{V,H}(e) \cap (R \cup F) = \emptyset$
\end{enumerate}
\end{theorem}
...
\begin{description}
  ...
  &\text{Looking at $x$, we have: }\\
  &locs_{V,H}(x) \nsubseteq R \cup locs_{V,H}(e_2) \tag{Heap linearity}\\
  &\text{Then,}\\
  &locs_{V,H}(x) \subseteq g \tag{def of $g$}\\
  &\text{Furthermore,}\\
