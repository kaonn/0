...
Next, we define the predicates $\ms{no\_alias}$: 
\begin{description}
% \item [$\ms{forest}(H)$: ]\\
...
\end{enumerate}
\item[$\ms{no\_ref}(V,H,v)$: ] There is no such $x \in \overline V$ s.t. $l \in reach_{H}(v) \cap \bigcup_{x \in \overline V} reach_{H}(V(x))$.\\
\end{description}
...
\begin{lemma}
For all stacks $V$ and heaps $H$, if $\na{V,H}$, $\Sigma^{\dagger}; \Gamma^{\dagger} \vdash e : B^{\dagger}$, $F \cap R = \emptyset$, $(F \cup R) \cap locs_{V,H}(e) = \emptyset$, $H \vDash V : \Gamma$, and $V,H,R,F \; \vdash e \Downarrow v, H', F'$, then $\ms{set}(reach_{H'}(v))$, $R \cap reach_{H'}(v) = \emptyset$, $F' \cap R = \emptyset$, $\ms{no\_ref}(V,H,v)$, and $\na{V,H'}$.
\end{lemma}
...
\begin{description}
  ...
  &\Sigma; \Gamma_1,\Gamma_2 \sststile{q'}{q} \irl{let}(e_1; x : \tau.e_2) : B \tag{case}\\
  &\Sigma; \Gamma_1 \sststile{p}{q} e_1 : A \tag{ad.}\\
  &\Sigma^{\dagger}; \Gamma_1^{\dagger} \vdash e_1 : A^{\dagger} \tag{Lemma 1.1}\\
  &\Sigma; \Gamma_2, x : A \sststile{q'}{p} e_2 : B \tag{ad.}\\
  &\text{Suppose } \na{V,H}, F \cap R = \emptyset set, (F \cup R) \cap locs_{V,H}(e) = \emptyset, \text{ and } H \vDash V : \Gamma\\
  &H \vDash V : \Gamma_1 \tag{def of W.D.E}\\
  &F \cap R' = \emptyset \text{ since } F \cap locs_{V,H}(e) = \emptyset\\
  &R' \cap locs_{V,H}(e_1) = \emptyset \tag{$\na{V,H}$}\\
  &(F \cup R') \cap locs_{V,H}(e_1) = \emptyset \tag{Sp.}\\
  &\text{By IH, } \ms{set}(reach_{H_1}(v_1)), R' \cap reach_{H_1}(v_1) = \emptyset, F_1 \cap R' = \emptyset, \text{ and } \na{V,H_1}\\
  &
  \end{align*}
  ...
  &\text{Suppose }  \na{V,H}, F \cap R = \emptyset, \text{ and } H \vDash V : \Gamma\\
  ...
  &\quad \text{AFSOC, suppose } l' \in r_{x_1} \cap r_{x_2}\\
  &\quad \text{but then } \mu_{reach_H(l)}(l') \ge 2, \text{ and } \ms{set}(H(l)) does not hold.\\
  &\quad \text{hence } r_{x_1} \cap r_{x_2} = \emptyset\\
  &\quad \textbf{case: otherwise} \\
  &\quad \text{similar to the above}\\
  &\text{Thus we have } \na{V'',H}\\
  &(F \cup g) \cap R = \emptyset \text{ since } F \cap R = \emptyset \tag{def of $g$}\\
  &\text{By IH, } \ms{set}(reach_{H'}(v)), F' \cap R = \emptyset, \text{ and } \na{V'',H'}\\
  &\text{NTS } \na{V,H'}\\
  &\text{Follows from Lemma... }\\
  \end{align*}
  \end{description}
\end{proof}
