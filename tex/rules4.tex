...
$\forall l \in H$, $\forall l_1 \in R_1, l_2 \in R_2$, let $H \vDash p : l_1 \leadsto l$ and $H \vDash q : l_2 \leadsto l$, then $l_1 = l_2$ and $H \vDash p \equiv q : l_1 \leadsto l$.
\item[$\na{V}{H}$: ] \\
$\forall l \in H$, $\forall x,y \in \overline V$, x \ne y. Let $r_x = root(\overline V(x))$, $r_y = root(\overline V(y))$. Then:
\begin{enumerate}[(1)]
