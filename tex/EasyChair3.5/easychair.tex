...
set $R$. Now which introduce the sharing rule, which is why $\ms{copy}$ is so named:
...
Before proving the soundness of $\fogc$ w.r.t. $\ms{copy}$, we need to show that $\ms{copy}$ is 
a linear, i.e. locations are used linearly. 
...
Utimately, we want to prove the soundess of the type system w.r.t \ms{free}:
...
Where the dashed arrow represents the intended result. With the linearity of $\ms{copy}$ in place,
we are ready to show the soundness of $\fogc$ w.r.t \ms{copy}. To formally state this theorem,
...
In this case, the ``freelist'' is the whole ambient set of locations $\ms{Loc}$, 
