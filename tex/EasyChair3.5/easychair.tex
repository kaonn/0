...
be found in the appendix~\ref{sect:appendix}.
...
\inferr{
  ...
  V'[x_1 \mapsto v',x_2 \mapsto v'],H',R,F \cup g \; \vdash e \Downarrow v,H'',F'
}(\text{F:Share})
\end{mathpar}
...
\begin{align*}
	&\texttt{append} : \prodtycst{L^p(\irl{int})}{L^q(\irl{int})} \xrightarrow{r/r} L^s(\irl{int}) \text{ such that }
	p \ge s + 1 \text{ and } q \ge s\\
	&\texttt{appTwice} : L^p(\irl{int}) \xrightarrow{q/q} \prodtycst{L^r(\irl{int})}{L^s(\irl{int})}
	\text{ such that } p \ge r + s + 2
\end{align*}
...
\textbf{Linear Potential Functions}\label{sect:LPF}
...
semantic values (structures defined in Section~\ref{sect:lfp}).
...
are already in the freelist. Although we can leave this condition out, it unnecessary 
...
we have have that $F_1$ is greater $F_2$, with the overhead $\oh{\gamma}$ being exactly the sum 
...
When comparing evaluation results between a run with \copySem{} and $\mathcal{E}_{\ms{oper}}$, as the return values 
...
will be sufficiently large (as predicated by the type derivation). The 5-place predicate $\ms{linear}$ 
