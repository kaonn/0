...
language community has developed automatic and semi-automatic
analysis techniques.  \jan{heap-space bounds}
%
These systems are often special cases of more general resource bound
analyses that are based on abstract interpretation~\cite, recurrence
solving~\cite{}, type systems~\cite{}, or term rewriting~\cite{}.
...
or simply ignore deallocation in the analysis~\cite{}. As a result,
the derived bounds are not accurate when the underlying system employs
garbage collection. The only exceptions we are aware of are the works
by Albert et al.~\cite{Albert09,Albert13}, by Braberman et
al.~\cite{Braberman08}, and Unnikrishnan et
...
that it is based on a type system and that it is proved sound with
respect to a formal cost semantics. Advantages of a type based
approach include its natural compositionality and the type derivations
as certificates for the resource bounds.
...
$(y_f,e_f)_{f \in \Sigma}$, where $\Sigma(y_f) = A \to B$, 
and $\Sigma;y_f : A \vdash e_f : B$ (the type system is discussed in $\ref{sect:typing}$). 
...
The type system is based on the affine type system in \cite{Hoffmann:2017:TAR:3009837.3009842}. 
