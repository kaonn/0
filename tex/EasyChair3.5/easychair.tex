...
For \texttt{appendTwice}, we first share the list \texttt{l}, which prevents the 
first call to \texttt{append} from destructing \texttt{l1}. Thus the overhead is 
the size of the input. The second call has no overhead since \texttt{l2} can be 
destructed. The return value is a pair which is stack-allocated and doesn't require a heap 
