 ...
 \inferrule*[right=\begin{tiny}\text{L:Fun}\end{tiny}]
   {
    ...
    \Sigma(\texttt{append}) = \prodtycst{L^s(A)}{L^s(A)} \xrightarrow{q/q} L^s(A)W
   }{
...
\begin{definition}
  ...
  Given a valid sequence $P$ w.r.t. $V(x)$,
	...
	and $reach_H(V(x;P))$ for $reach_H(V_H(x;P))$.
        Given a map $m : X \to \mathcal{S}(\ms{dir})$ that maps each
        $x \in X \subseteq dom(V)$ to a sequence $P$ valid w.r.t. $V(x)$,  
...
Below are the expected properties of a copy extension: 
\begin{lemma}
