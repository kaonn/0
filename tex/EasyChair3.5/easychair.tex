...
For example, we can give the following derivations for \texttt{append} and \texttt{appTwice}, where $A = \irl{int}$:
...
\inferrule*[vdots=5.5em,rightskip=12.5em]{
...
&
 ...
 \inferrule*[right=\begin{tiny}\text{L:Fun}\end{tiny}]
   {
...
Where $e_{\texttt{append}}$ denotes the body of the function $\texttt{append}$. The mapping
$P =  [\texttt{append} \mapsto e_{\texttt{append}}]$ is the program, and $\Sigma = [\texttt{append} \mapsto \prodtycst{L^p(\irl{int})}{L^p(\irl{int})} \xrightarrow{q/q} L^p(\irl{int})]$
...
we instantiate the previous type system this metric
...
\gamma' : dom(Y) \setminus (M \cup j) \to \mathcal{P}(dom(H)\setminus (F \cup g))\\
  ...
  since $V,H,R,F$ is a well defined configuration, we have $collect(R,reach_H(V),H,F) = \emptyset$.
  ...
  $|M| + |j| = |F| + | g| + |\oh{\gamma'}|$.
	By assumption, $|M| = |F| + |\oh{\gamma}|$, so it suffices to show 
        ...
        $\oplus$ on the left as a convenient way to distinguish between locations from
        $dom(Y)$ and $dom(H)$. 
