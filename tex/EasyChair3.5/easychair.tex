...
\begin{abstract}
  ...
  system. Predicting and formally analyzing the memory footprint is a
  challenging problem, particularly for high-level languages with
  automatic memory management.
  %
  However, most existing techniques that support developers by
  ...
  deallocation in the analysis. As a result, the derived memory bounds
  ...
  collection.
  %
  The cost model is based on a perfect garbage collector that
  ...
  %
  The first technical contribution of the work is an extension of AARA
  ...
  %
  The second technical contribution is a new operational cost
  semantics that models a perfect garbage collector and the soundness
  proof of the extended AARA with respect to this cost semantics.
  %
...
$collect(\mathca{C} \vdash v,H',F') = \{l \in H' \mid l \notin F' \cup R \cup reach_H'(v)\}$. 
Now the key lemma:
