...
$\ms{Heap} \times \ms{Loc} \times \ms{Val} \times \ms{Heap} \times \ms{Val}$. We write this as 
...
$\ms{Stack} \times \ms{Heap} \times \ms{\{Loc\}} \times \ms{\{Loc\}}$, usually written as $V,H,R,F$. 
P is a program with signature $\Sigma : \ms{Var} \to \ms{FTypes}$. 
This can be read as: under stack $V$, heap $H$, roots $R$,
freelist $F$, and program $P$ with signature $\Sigma$, the expression $e$ evaluates to $v$, and engenders a new heap $H'$ and freelist $F'$. Here, $\ms{Stack}$ is defined as the set of finite mappings $\ms{Var} \to \ms{Val}$, and $\ms{Heap}$ is defined as the set of finite mappings $\ms{Loc} \to \ms{Val}$.\\
...
\infern{
  ...
  |L| = \ssize{H}{v'}\\
  H',v'' = copy(H,L,v')\\
	V_2 = V'[x_1 \mapsto v',x_2 \mapsto v'']\\
 	V_2,H',R,F \setminus L \; \vdash e \Downarrow v,H'',F'
}{
...
\begin{definition}
A configuration $\mathcal{C}_1 = (\mathcal{S}_1,R_1,F_1)$ is less than a configuration
$\mathcal{C}_2 = (\mathcal{S}_2,R_2,F_2)$ iff
\begin{enumerate}
\item $|F_1| \le |F_2|$
\item $R_1 = R_2$
\item $\mathcal{S}_1 \sim \mathcal{S}_2$
\end{enumerate}
Write this as $\mathcal{C}_1 \le \mathcal{C}_2$.
\end{definition} 

Now the lemma:

\begin{lemma}
Let $\mathcal{C}_1 \vdash^{\mathsf{copy}} e \Downarrow v,H',F'$, and $H' \vDash v \mapsto a : A$.
Then for all configurations $\mathcal{C}_2$ such that $\mathcal{C}_2 \le \mathcal{C}_1$,
there is exists a triple
$(w,Y',M') \in \ms{Val} \times \ms{Heap} \times \ms{Loc}$ s.t.
	\begin{enumerate}
			\item $\mathcal{C}_2 \vdash^{\mathsf{free}} e \Downarrow w,Y',M'$
			\item $\veq{H'}{Y'}{v}{w}$
			\item $|M'| \ge |F'|$
	\end{enumerate}
\end{lemma}
...
\begin{description}
  ...
  \begin{align*}
		...
		&\text{Suppose } H \vDash V(x) \mapsto a : A.
			\text{ We need to show a triple that satisfies the 3 post-conditions.}\\
		&\text{Take } (v',H'',F'') = (V(x),H,F)\\
		&\emph{(1)}\quad V,H,R,F \vdash^{\mathsf{free}} e \Downarrow v',H'',F'' \tag{F:Var}\\
		&\emph{(2)}\quad H'' \vDash v' \mapsto a : A \tag{assumption}\\
		&\emph{(3)}\quad |F''| = |F| \ge |F|
  \end{align*}
  \item[Case 2: E:Const*]
  Due to similarity, we show only for E:ConstI
  \begin{align*}
  ...
  \begin{align*}
		...
		&\text{Suppose } H' \vDash v \mapsto a : A. 
			\text{ We need to show a triple that satisfies the 3 post-conditions.}\\
		&\text{By IH, we have } (v',H'',F'') \text{ such that }\\
		&\emph{(1)}\quad V',H,R,F \cup g \vdash^{\mathsf{free}} e_1 \Downarrow v',H'',F''\\
		&\emph{(2)}\quad H'' \vDash v' \mapsto a : A \\
		&\emph{(3)}\quad |F''| \ge |F'|\\
...
$\Sigma = [\texttt{map} \mapsto \prodtycst{A \to B}{\listtyabt{A}} \to \listtyabt{B}]$\\
