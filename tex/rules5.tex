...
\infern{
  ...
  |L| = \ssize{H}{v'}\\
  H',v'' = copy(H,L,v')\\
  V'[x_1 \mapsto v',x_2 \mapsto v''],H',R,F \setminus L \; \vdash e \Downarrow v,H'',F'
}{
...
an equal or smaller freelist. This is expressed as the following lemma:

\begin{lemma}
Let $V,H,R,F \vdash^{\mathsf{copy}} v,H',F'$, and $H \vDash v \mapsto a : A$.
Then there exists an $F''$ s.t.
	\begin{enumerate}
			\item $V,H,R,F \vdash^{\mathsf{free}} v',H',F''$
			\item $H \vDash v' \mapsto a : A$
			\item $|F''| \ge |F'|$
	\end{enumerate}
\end{lemma}
